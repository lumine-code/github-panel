...
export default class ReviewsView extends React.Component {
  ...
  // tabIndex so the keymap's `.github-panel-Reviews` block can fire. The host
  // pane item is a bare div, so without it activating the tab leaves
  // document.activeElement outside the panel and the bindings only work once
  // something inside has been clicked. `.github-panel-GitHub` does the same.
  render() {
    return (
      <div className="github-panel-Reviews" tabIndex="-1" ref={this.rootHolder.setter}>
        {this.renderCommands()}
        {this.renderHeader()}
    ...
    return (
      <Fragment>
        <Commands registry={this.props.commands} target={this.rootHolder}>
          <Command
            command="github-panel:more-context"
            description="Show more unchanged lines around each reviewed hunk."
            callback={this.props.moreContext}
          />
          <Command
            command="github-panel:less-context"
            description="Show fewer unchanged lines around each reviewed hunk."
