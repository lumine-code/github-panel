...
export function openIssueishLinkInNewTab(url, options = {}) {
  const uri = getLumineUriForGithubUrl(url);
  if (uri) {
...
function getLumineUriForGithubUrl(githubUrl) {
...
export function getDataFromGithubUrl(githubUrl) {
  ...
  let pathname = "";
  try {
    ({ hostname, pathname } = new URL(githubUrl));
  } catch {
    // Not a parseable absolute URL — leave the fields empty so getUriForData rejects it.
  }
...
function openInNewTab(uri, { activate } = { activate: true }) {
  return lumine.workspace.open(uri, { activateItem: activate });
}
