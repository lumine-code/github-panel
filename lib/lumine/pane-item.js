...
/**
 * PaneItem registers an opener with the current workspace as long as this component is mounted. The opener will
 ...
 * render() {
 *   return (
 *     <PaneItem workspace={this.props.workspace} uriPattern='lumine-github://host/{id}'>
 *       {({itemHolder, params}) => (
...
export default class PaneItem extends React.Component {
  ...
  render() {
    return this.state.currentlyOpen.map((item) => {
      return <Fragment key={item.getKey()}>{item.renderPortal(this.props.children)}</Fragment>;
    });
  }
