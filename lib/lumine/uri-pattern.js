 ...
 * Examples:
 *
 * `lumine-github://hostname/exact/path?p0=value&p1=value` contains no parameters, so it will match _only_ that exact URL,
 * including query parameters and their values. Extra query parameters or a fragment (`#`) will cause the match to fail.
 *
 * `lumine-github://hostname/path/{name}/fragment` will match and capture any second path segment.
 * * `lumine-github://hostname/path/one/fragment` will match with `{name: 'one'}`
 * * `lumine-github://hostname/path/two/fragment` will match with `{name: 'two'}`
 * * `lumine-github://hostname/path/fragment` will not.
 *
 * `lumine-github://hostname/root/{segments...}` will capture any number of path segments as an array. For example,
 * * `lumine-github://hostname/root/foo/bar/baz/` will match with `{segments: ['foo', 'bar', 'baz']}`.
 * * `lumine-github://hostname/root/single` will match with `{segments: ['single']}`; even a single segment will be
 *   matched as an array.
 * * `lumine-github://hostname/root/` will match with `{segments: []}`.
 *
 * Query parameters and their values may be captured. Given: `lumine-github://hostname?q={value}`
 * * `lumine-github://hostname?q=foo` will match with `{value: 'foo'}`.
 * * `lumine-github://hostname?q=one&q=two` will _not_ match.
 *
 * To match multiple query parameters, use a splat parameter. Given: `lumine-github://hostname?q={value...}`
 * * `lumine-github://hostname?q=one&q=two` will match with `{value: ['one', 'two']}`.
 * * `lumine-github://hostname?q=single` will match with `{value: ['single']}`.
 * * `lumine-github://hostname` will match with `{value: []}`.
 *
...
export default class URIPattern {
  constructor(string) {
    ...
    const parsed = parseURI(dashEscape(string));
    this.parts = {
...
/**
 * Parse a URI into the shape URIPattern needs using the WHATWG URL API instead of the deprecated `url.parse()`:
 * `protocol` (with trailing `:`), `auth` (`"user:pass"` or null), `hostname`, `port`, `pathname`, a `query` object whose
 * repeated keys become arrays (matching `url.parse(str, true)`), and `hash` (with leading `#`). An unparseable string
 * yields empty parts so it simply fails to match, mirroring the old lenient behavior.
 */
function parseURI(string) {
  ...
  let auth = null;
  if (parsed.username) {
    auth = parsed.password ? `${parsed.username}:${parsed.password}` : parsed.username;
  }
...
/**
 * Including `{}` characters in certain URI components (hostname, protocol) cause the URL parser to lump everything into
