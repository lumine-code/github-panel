...
function getStubItem() {
  if (!StubItem) {
    const stubItemModule = require("./items/stub-item");
    StubItem = stubItemModule.default || stubItemModule;
  }
  return StubItem;
}
...
export default class GithubPanelPackage {
  ...
  async activate() {
    ...
    if (this.startOpenGitHubTab) {
      ...
      // the GitHub tab is closed, so keep React and that controller graph off
    ...
    this.renderFn(
      <GitHubRootController
        ...
        setContextLock={(p, lock) =>
          lumine.repositories.setActiveRepositoryForPath(p, { pin: lock })
        }
  ...
  createDockItemStub({ uri }) {
    if (uri !== "lumine-github://dock-item/github") {
