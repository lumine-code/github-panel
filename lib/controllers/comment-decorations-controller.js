...
export class BareCommentDecorationsController extends React.Component {
  ...
  render() {
    // The command registration comes first and unconditionally: it is in a
    // menu, and a menu item whose command exists only when a pull request
    // happens to be checked out looks broken rather than unavailable. Both
    // early returns below leave it in place, and openReviewsTab says why it
    // declined.
    const commands = (
      <Commands registry={this.props.commands} target="lumine-workspace">
        <Command
          command="github-panel:open-reviews-tab"
          description="Open the review comments of the pull request being viewed."
  ...
  openReviewsTab = () => {
    ...
    if (!pullRequest) {
      lumine.notifications.addWarning("GitHub: no pull request to review", {
        detail: "Check out a branch with an open pull request first.",
      });
      return null;
    }
